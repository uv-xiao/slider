...
Create a professional presentation slide image. Follow the user's slide specification strictly.

SLIDE LAYOUT (16:9 aspect ratio):
- Respect requested regions, bounding boxes, and alignment.
- Keep safe margins unless the spec calls for full-bleed imagery.
- Maintain clear visual hierarchy and consistent spacing.

TYPOGRAPHY:
- Ensure all text is legible when projected.
- Prefer short, well-structured text blocks over paragraphs unless the spec requires otherwise.
- Do not invent author/presenter names or extra footer text unless explicitly specified.

VISUAL ELEMENTS:
- Render icons/illustrations/diagrams/tables exactly as described in the spec.
- If the spec includes an element inventory with positions, treat it as the source of truth.
- Avoid adding extra decorative elements that are not requested.

PRESENTATION QUALITY:
- Consistent styling across slides (reuse visual language from previous slide when given a previous-slide image).
- High contrast and clean composition, but allow vivid/intuitive elements when specified.
"""

    # Guidelines for generating slide visuals only (figures/images for PPT)
    VISUAL_ONLY_GUIDELINES = """
Create a high-quality visual/figure for embedding in a slide.

- Follow the user's visual specification exactly (shapes, labels, colors, layout).
- Use a clean background unless the spec requests otherwise.
- Keep text legible and avoid unrequested decorative noise.
"""
