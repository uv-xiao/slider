...
<IMPORTANT>
If we already have full per-page prompt of the slides, skip the planning stage, and the remaining "design and creation", "content development", "visual validation", and any other remaining tasks must be based on and observe the input per-page prompt. That is, you must guarantee the contents from per-page prompt are kept, which can be kept as text, or be presented as equal-meaning illustration.
</IMPORTANT>
...
3. **Include citations directly in the prompt** for slides that reference research:
...
4. **Attach existing figures/data for results slides** (CRITICAL for data-driven presentations):
